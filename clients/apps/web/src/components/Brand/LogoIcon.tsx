...
const LogoIcon = ({
  className,
  size = 29, // Default size is 29 (used for fallback)
}: {
  className?: string
  size?: number // Size in pixels (fallback)
}) => {
  return (
    <svg
      width="10%" // Make the width 100% to allow it to scale
      height="10%" // Make the height 100% to allow it to scale
      viewBox="0 0 57.6 65" // Adjusted viewBox for the full SVG dimensions
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={twMerge(className ? className : '')} // Merge additional classes if provided
    >
      <style>
        {`
