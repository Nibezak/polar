---
title: Integrating Polar with Laravel
description: Integrate Polar with your Laravel application
keywords: laravel, php, polar, integrate, sell, products, payments, storefront, embedded checkout, api, example, tutorial, guide
---
...
![Polar x Laravel](./polar-laravel.jpg)
...
<GitHubIcon width={40} />
<h3>Polar Laravel Example App</h3>
<p>We've created a simple example Laravel application that you can use as a reference</p>
...
To authenticate with Polar, you need create an access token, and supply it to Laravel using a `POLAR_API_KEY` environment variable.
...
Products are tied to organizations, not your personal account. You need to supply the organization ID to Laravel using a `POLAR_ORGANIZATION_ID` environment variable.
Organization IDs for a given organization can be found on the organization's settings page.

```bash
# .env
POLAR_API_KEY="payflow_pat..."
